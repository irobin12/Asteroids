...
public class RockData : EntityData
{
    ...
    [Tooltip("Number of rocks to spawn.")] public int spawnedRocksAmount = 3;

    [Tooltip("Min angle at which the spawned rocks will deviate from this rock.")] [Range(0, 360)]
    public int minSpawnAngleDeviation = 10;

    [Tooltip("Max angle at which the spawned rocks will deviate from this rock.")] [Range(0, 360)]
    public int maxSpawnAngleDeviation = 90;

    [Tooltip(
        "Min velocity multiplier with which the spawned rocks will deviate from this rock, 1 being the same speed as the parent rock.")]
    [Range(0.5f, 1f)]
    public float minVelocityMultiplier = 0.9f;

    [Tooltip(
        "Max velocity multiplier with which the spawned rocks will deviate from this rock, 1 being the same speed as the parent rock.")]
    [Range(1f, 1.5f)]
